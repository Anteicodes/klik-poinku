...
echo color("purple", "| -------------------------------- \n");
echo color("red","| 1. Login (PERBAIKAN) \n");
echo color("nevy","| 2. Daftar \n");
...
//     echo "$nomer : ".$str.PHP_EOL;}
    ...
    $produk = [
        ...
        "SOV2L" => "fc4b5152-70a1-4344-9fb8-fffb43276a20",
    ...
    $url = "https://www.klikindomaret.com/checkout/addvoucher";

    $curl = curl_init($url);
    curl_setopt($curl, CURLOPT_URL, $url);
    curl_setopt($curl, CURLOPT_POST, true);
    curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($curl, CURLOPT_HTTPHEADER, $header_sudah_token2);

    $data = "code=$idproduk_input";
...
elseif($inputan == "5"){
    ...
    if(file_exists("config.json")){
        ...
        echo color("yellow","| Pass : $ar1 \n");
    go:
        echo color("yellow","| Masukan Nomer Hpnya: ");
        $input_hp = fopen("php://stdin","r");
        ...
        echo color("yellow","| Pesan : $pesan_login").PHP_EOL;
        if ($pesan_login == "Phone Number not valid"){die;}

    /////////////////////////////////////////
            otp_point:
            echo color("yellow","| Masukan Kode OTP Indo-Point : ");
            $input_otp = fopen("php://stdin","r");
            $otp_point = trim(fgets($input_otp));
            $otp = "\"$otp\"";
            ...
            if ($status == '01'){}else{goto otp_point;}
            $token = $minta_token['data']['access_token'];
            $cek_user = $minta_token['data']['isNewRegister'];
            if (empty($cek_user)){ 
                echo color("yellow","| Anda login menggunakan akun lama").PHP_EOL;
            }else {
                $header_token = [
                    'user-agent: okhttp/4.9.0',
                ...
                echo color("green","| KUPON SUKSES DIAMBIL BOSS!!!").PHP_EOL;
                echo color("green","| Untuk melihat Kupon ketik cat kupon.txt").PHP_EOL;
                $kupon_klik = $hasil_voucher['data']['content']['6']['couponCode'];
                ...
                if($hasil_cek !== "[]"){ echo color("red","| Nomor Sudah Terdaftar Silahkan Login \n"); goto welcome;}
                    ...
                    if ($hasil_otp2 == "Kode OTP sudah expired." || $hasil_otp2 == "Kode OTP salah." ){ echo color("red","| $hasil_otp2 \n"); goto otpverif2;}elseif($code == "500"){
                ...
                //     echo "$nomer : ".$str.PHP_EOL;}
                    ...
                    $hasil_cek_troli = json_decode($hasil_cek_troli,true);
                    if ($hasil_cek_troli['Quantity'] == "0"){ 
                        echo color("yellow","| Troli anda kosong! Pastikan kode toko benar!!"); goto welcome; }
                    else{
                    echo color("green",'| Troli Anda '.$hasil_cek_troli['Quantity']."\n");
